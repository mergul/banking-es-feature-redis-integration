        ...
        match &result {
            ...
            Err(e) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                error!(
                    "Failed to deposit money: {} amount {} error {} in {:?}",
                    account_id,
                    amount,
                    e,
                    start_time.elapsed()
        ...
        match &result {
            ...
            Err(e) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                error!(
                    "Failed to withdraw money: {} amount {} error {} in {:?}",
                    account_id,
                    amount,
                    e,
                    start_time.elapsed()
        ...
        match &result {
            ...
            Err(e) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                error!(
                    "Failed to close account: {} error {} in {:?}",
                    account_id,
                    e,
                    start_time.elapsed()
                );
        ...
        match &result {
            Ok(Some(_)) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                info!(
                    "Account retrieved successfully: {} in {:?}",
                    account_id,
                    start_time.elapsed()
                );
            }
            ...
            Err(e) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                error!(
                    "Failed to get account: {} error {} in {:?}",
                    account_id,
                    e,
                    start_time.elapsed()
                );
        ...
        match &result {
            Ok(accounts) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                info!(
                    "All accounts retrieved successfully: {} accounts in {:?}",
                    accounts.len(),
                    start_time.elapsed()
                );
            }
        ...
        match &result {
            Ok(transactions) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                info!(
                    "Account transactions retrieved successfully: {} transactions in {:?}",
                    transactions.len(),
                    start_time.elapsed()
                );
            }
            Err(e) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                error!(
                    "Failed to get account transactions: {} error {} in {:?}",
                    account_id,
                    e,
                    start_time.elapsed()
                );
        ...
        match &result {
            Ok(balance) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                info!(
                    "Account balance retrieved successfully: {} balance {} in {:?}",
                    account_id,
                    balance,
                    start_time.elapsed()
                );
            ...
            Err(e) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                error!(
                    "Failed to get account balance: {} error {} in {:?}",
                    account_id,
                    e,
                    start_time.elapsed()
                );
        ...
        match &result {
            Ok(is_active) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                info!(
                    "Account active status checked: {} active {} in {:?}",
                    account_id,
                    is_active,
                    start_time.elapsed()
                );
            ...
            Err(e) => {
                self.metrics
                    ...
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                error!(
                    "Failed to check account active status: {} error {} in {:?}",
                    account_id,
                    e,
                    start_time.elapsed()
                );
