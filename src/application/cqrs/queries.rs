                        ...
                        match self.convert_projection_to_account(&projection) {
                            Ok(account_to_cache) => {
                                ...
                                match self.cache_service
                                    .set_account(&account_to_cache, Some(std::time::Duration::from_secs(3600)))
                                    .await {
                                    Ok(_) => info!("[AccountQueryHandler] Successfully cached account {}.", account_id),
                                    Err(e) => {
                                        error!("[AccountQueryHandler] Failed to cache account {}: {}", account_id, e);
                ...
                match self.convert_transactions_to_events(&transactions) {
                    Ok(events_to_cache) => {
                        ...
                        match self.cache_service
                            ...
                            .await {
                            Ok(_) => info!("[AccountQueryHandler] Successfully cached account events for {}.", account_id),
                            Err(e) => {
