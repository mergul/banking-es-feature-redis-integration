...
impl ConsistencyManager {
    ...
    pub async fn wait_for_consistency(&self, aggregate_id: Uuid) -> Result<()> {
        info!("[ConsistencyManager] Waiting for CDC consistency for aggregate {}", aggregate_id);
    ...
    pub async fn wait_for_projection_sync(&self, aggregate_id: Uuid) -> Result<()> {
        info!("[ConsistencyManager] Waiting for projection sync for aggregate {}", aggregate_id);
        self.wait_for_status(
            aggregate_id,
        ...
        loop {
            ...
            if let Some(status) = status {
                if is_completed(&status) {
                    info!("[ConsistencyManager] {} completed for aggregate {}", wait_description, aggregate_id);
                    return Ok(());
                }
                if let Some(error) = get_error(&status) {
                    warn!("[ConsistencyManager] {} failed for aggregate {}: {}", wait_description, aggregate_id, error);
                    return Err(anyhow::anyhow!("{} failed for aggregate {}: {}", wait_description, aggregate_id, error));
                }
            }

            if start_time.elapsed() >= self.max_wait_time {
                warn!("[ConsistencyManager] TIMEOUT waiting for {} for aggregate {} after {:?}", wait_description, aggregate_id, self.max_wait_time);
                return Err(anyhow::anyhow!("Timeout waiting for {} for aggregate {} after {:?}", wait_description, aggregate_id, self.max_wait_time));
            }

            match tokio::time::timeout(self.max_wait_time - start_time.elapsed(), notifier.notified()).await {
