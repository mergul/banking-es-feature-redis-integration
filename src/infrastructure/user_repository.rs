...
impl UserRepository {
    ...
    pub async fn create(&self, new_user: &NewUser) -> Result<User, UserRepositoryError> {
        ...
        .map_err(|e: sqlx::Error| {
            if let Some(db_err) = e.as_database_error() {
                if let Some(pg_err) = db_err.try_downcast_ref::<PgDatabaseError>() {
                    if pg_err.code() == "23505" { // unique_violation
                        ...
                        if let Some(constraint_name) = pg_err.constraint() {
                            if constraint_name == "users_username_key" {
                                return UserRepositoryError::UsernameExists(new_user.username.clone());
                            }
                            if constraint_name == "users_email_key" {
                                return UserRepositoryError::EmailExists(new_user.email.clone().unwrap_or_default());
                            }
                        }
                    }
                }
