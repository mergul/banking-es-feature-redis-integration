...
impl CacheService {
    ...
    pub async fn get_account(&self, account_id: Uuid) -> Result<Option<Account>> {
        ...
        info!("[CacheService] Attempting to GET account from Redis with key: {}", key);

        match conn.get(key.as_bytes()).await {
            Ok(redis::Value::Data(data)) => {
                info!("[CacheService] Successfully GOT data from Redis for key: {}", key);
                match bincode::deserialize::<Account>(&data) {
                    Ok(account) => {
                        info!("[CacheService] Successfully deserialized account for key: {}", key);
                        self.metrics
                            .hits
                            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            ...
            Ok(redis::Value::Nil) => {
                info!("[CacheService] GET from Redis for key {} returned Nil (cache miss)", key);
                self.metrics
                    .misses
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                Ok(None)
            ...
            Err(e) => {
                self.metrics
                    .errors
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                error!("[CacheService] Redis GET operation error for key {}: {}", key, e);
                Err(e.into())
            }
        }
    }
    ...
    pub async fn set_account(&self, account: &Account, ttl: Option<Duration>) -> Result<()> {
        ...
        info!("[CacheService] Attempting to SET account in Redis for key: {} with TTL: {:?}", key, ttl_duration);

        let serialized_account = match bincode::serialize(account) {
            Ok(value) => {
                info!("[CacheService] Successfully serialized account for key: {}", key);
                value
            }
            Err(e) => {
                error!("[CacheService] Failed to serialize account for key {}: {}", key, e);
                return Err(e.into());
            }
        };
        ...
        let mut conn = self.redis_client.get_connection().await
            .map_err(|e| {
                error!("[CacheService] Failed to get Redis connection for SET: {}", e);
                e
            })?;
        use redis::AsyncCommands;
        ...
        pipeline.set_ex(key.as_bytes(), &serialized_account, ttl_duration.as_secs() as u64);
        ...
        info!("[CacheService] Updated in-memory cache for account: {}", account.id);
    ...
    pub async fn get_account_events(&self, account_id: Uuid) -> Result<Option<Vec<AccountEvent>>> {
        ...
        if let Some(events_entry) = self.event_cache.get(&account_id) {
            info!("[CacheService] In-memory cache HIT for account events: {}", account_id);
            return Ok(Some(
                events_entry.iter().map(|(_, event)| event.clone()).collect(),
            ));
        }
        info!("[CacheService] In-memory cache MISS for account events: {}", account_id);

        let mut conn = self.redis_client.get_connection().await
            .map_err(|e| {
                error!("[CacheService] Failed to get Redis connection for GET account events: {}", e);
                e
            })?;
        use redis::AsyncCommands;
        let key = format!("events:{}", account_id);
        info!("[CacheService] Attempting to GET account events from Redis with key: {}", key);

        match conn.get(key.as_bytes()).await {
            Ok(redis::Value::Data(data)) => {
                info!("[CacheService] Successfully GOT data from Redis for account events key: {}", key);
                match bincode::deserialize::<Vec<(i64, AccountEvent)>>(&data) {
                    Ok(events) => {
                        info!("[CacheService] Successfully deserialized account events for key: {}", key);
                        self.metrics
                            .hits
                            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                        // Update in-memory cache
                        self.event_cache.insert(account_id, events.clone());
                        info!("[CacheService] Updated in-memory event cache for account: {}", account_id);
            ...
            Err(e) => {
                self.metrics
                    .errors
                    .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                error!("[CacheService] Redis GET operation error for account events key {}: {}", key, e);
                Err(e.into())
            }
        }
    }
    ...
    pub async fn set_account_events(
        ...
        let ttl_duration = ttl.unwrap_or(self.config.default_ttl);
        let key = format!("events:{}", account_id);
        info!("[CacheService] Attempting to SET account events in Redis for key: {} with TTL: {:?}", key, ttl_duration);

        let serialized_events = match bincode::serialize(events) {
            Ok(value) => {
                info!("[CacheService] Successfully serialized account events for key: {}", key);
                value
            }
            Err(e) => {
                error!("[CacheService] Failed to serialize account events for key {}: {}", key, e);
                return Err(e.into());
            }
        };
        ...
        let mut conn = self.redis_client.get_connection().await
            .map_err(|e| {
                error!("[CacheService] Failed to get Redis connection for SET account events: {}",e);
                e
            })?;
        use redis::AsyncCommands;

        match conn.set_ex::<_, _, ()>(key.as_bytes(), &serialized_events, ttl_duration.as_secs() as u64).await {
            Ok(()) => {
                info!("[CacheService] Successfully SET account events in Redis for key: {}", key);
            }
            Err(e) => {
                error!("[CacheService] Redis SET operation error for account events key {}: {}", key, e);
                return Err(e.into());
            }
        }
    ...
    pub async fn delete_account_events(&self, account_id: Uuid) -> Result<()> {
        ...
        use redis::AsyncCommands;
        let key = format!("events:{}", account_id);
        info!("[CacheService] Deleting account events from Redis for key: {}", key);
