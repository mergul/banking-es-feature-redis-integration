...
impl EventStore {
    ...
    async fn bulk_insert_events_copy(
        ...
        for event in &events {
            ...
            std::io::Write::write_all(&mut write_buf, &(event_type_bytes.len() as i32).to_be_bytes()).unwrap();
            std::io::Write::write_all(&mut write_buf, event_type_bytes).unwrap();

            // event_data
            std::io::Write::write_all(&mut write_buf, &(event.event_data.len() as i32).to_be_bytes()).unwrap();
            ...
            let pg_epoch = chrono::DateTime::parse_from_rfc3339("2000-01-01T00:00:00+00:00").unwrap().timestamp_micros();
            ...
            std::io::Write::write_all(&mut write_buf, &(metadata_bytes.len() as i32).to_be_bytes()).unwrap();
